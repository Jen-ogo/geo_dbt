...
with all_h3 as (
  select region_code, h3_point_to_cell_string({{ wkt_to_geog('geom_wkt_4326') }}, 10) as h3_r10
  from {{ ref('ev_chargers') }}
  where geom_wkt_4326 is not null

  union all
  select region_code, h3_point_to_cell_string(st_centroid({{ wkt_to_geog('geom_wkt_4326') }}), 10)
  from {{ ref('road_segments') }}
  where geom_wkt_4326 is not null

  union all
  select region_code, h3_point_to_cell_string({{ wkt_to_geog('geom_wkt_4326') }}, 10)
  from {{ ref('poi_points') }}
  where geom_wkt_4326 is not null
...
distinct_h3 as (
  select distinct region_code::string as region_code, h3_r10::string as h3_r10
  from all_h3
